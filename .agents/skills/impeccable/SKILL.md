...
> **CRITICAL ANTIGRAVITY ENVIRONMENT RULE:**
> This project is "Midnight Club". You must follow the **FormulaMid 4.0** standards.
> 1. **Functional Brutalism**: UI must be an operational control panel, not a marketing site.
> 2. **Data Density**: High density is required for POS terminals and logistics. Do not add whitespace or "clean" designs that reduce data on screen.
> 3. **No Marketing Fluff**: Do not add unnecessary animations or gradients.
> 4. **Tokens.css**: Use `--token-` variables for all colors. Do not hardcode hex or use arbitrary Tailwind colors.
