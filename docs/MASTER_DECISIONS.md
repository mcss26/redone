...
### 2026-06-05: Integraci�n de Costos Fijos (Overheads)
**Context:** Se requer�a contabilizar los costos estructurales mensuales (alquiler, servicios, etc.) en el P&L que hasta el momento solo contemplaba costos operativos por jornada. Adem�s, se enfrentaban bloqueos del Supabase CLI local por dependencias de Docker.
**Decision:**
1. Crear una skill (\supabase-cli-executor.js\) utilizando la Management API de Supabase v�a \
ode-fetch\ para inyectar SQL remotamente y sortear dependencias del OS.
2. Desarrollar el m�dulo \FixedCostsModule.jsx\ (Functional Brutalism) para gestionar los registros mensuales e integrarlo al motor matem�tico de \MonthlyReportModule.jsx\, restando el total directamente del Net Profit mensual.
**Consequences:**
...
- **Cons:** El gasto estructural recae totalmente en la consolidaci�n mensual y no en una m�trica diaria prorrateada, lo que es correcto seg�n la visi�n ejecutiva pero requiere cerrar el mes para ver el impacto.
