...
import { Plus, X, Save, ArrowLeft, ChevronRight, CalendarDays, Trash2, Loader2, Search } from 'lucide-react';
...
export default function WorkDaysModule({ onNavigate }) {
  ...
  const STATUS_COLORS = {
    open: 'bg-brand-success/20 text-brand-success',
    closed: 'bg-brand-muted/50 text-brand-bg',
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xs font-extrabold tracking-[0.3em] uppercase text-brand-muted">JORNADAS (WORK DAYS)</h2>
              <p className="text-[10px] text-brand-muted/40 tracking-wide mt-0.5">Gestión operativa central</p>
            </div>
          </div>
          <button
            onClick={openCreate}
          ...
          <table className="w-full whitespace-nowrap">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={wd.id} onClick={() => openEdit(wd)} className="border-b border-brand-border/30 hover:bg-brand-card/50 transition-colors cursor-pointer group">
                  ...
                  <td className="px-5 py-3.5 text-center">
                    <span className={`px-2 py-1 rounded text-[10px] font-bold tracking-widest ${STATUS_COLORS[wd.status]}`}>
                      {STATUS_LABELS[wd.status]}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-right">
                    <button className="text-brand-muted group-hover:text-brand-text transition-colors cursor-pointer" title="Ver Detalles">
        ...
        <>
          ...
          <div className="absolute top-0 right-0 h-full w-full max-w-sm bg-brand-bg border-l border-brand-border z-50 flex flex-col animate-slide-in">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              {!selectedDay && (
                <div className="p-4 bg-brand-accent/10 border border-brand-accent/20 rounded-xl mb-6">
                  <p className="text-xs text-brand-accent leading-relaxed">
                    Al crear una jornada, se generará automáticamente en estado <strong>ABIERTA</strong>. Podrá recibir modificaciones hasta que sea cerrada operativamente al final de la noche.
                  </p>
                </div>
              )}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full bg-brand-surface border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-text focus:outline-none focus:border-brand-muted transition-colors resize-none"
                  placeholder="Instrucciones especiales para esta noche..."
                />
              </div>
