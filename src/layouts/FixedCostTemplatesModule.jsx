...
export default function FixedCostTemplatesModule({ onNavigate }) {
  ...
  const toggleActive = async (t) => {
    try {
      const { error } = await supabase.from('fixed_cost_templates').update({ active: !t.active }).eq('id', t.id);
      if (error) throw error;
      triggerFlash('success');
      fetchData();
    } catch (err) {
      ...
      <div className={`flex-1 overflow-y-auto p-6 md:p-8 ${isFetchingBackground ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="">
          <table className="w-full whitespace-nowrap">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="text-center py-12 text-brand-muted text-xs uppercase tracking-widest">Cargando...</td></tr>
              ) : templates.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-12 text-brand-muted/50 text-xs italic uppercase tracking-widest">El master estructural está vacío.</td></tr>
              ) : templates.map((t) => (
                <tr key={t.id} className="border-b border-brand-border/30 hover:bg-brand-card/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-center">
                    <button onClick={() => toggleActive(t)} className="cursor-pointer" title={t.active ? 'Activo (Poblará los meses)' : 'Inactivo'}>
                      <div className={`w-2 h-2 rounded-full mx-auto shadow-[0_0_6px_rgba(0,0,0,0.5)] ${t.active ? 'bg-brand-success' : 'bg-brand-error'}`} />
                    </button>
                  </td>
          ...
          <div className="absolute top-0 right-0 h-full w-full max-w-sm bg-brand-bg border-l border-brand-border z-50 flex flex-col animate-slide-in">
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-brand-border shrink-0">
              <h3 className="text-xs font-bold tracking-[0.2em] uppercase text-brand-muted">
                {slideOver === 'create' ? 'NUEVA ESTRUCTURA' : 'EDITAR ESTRUCTURA'}
              </h3>
