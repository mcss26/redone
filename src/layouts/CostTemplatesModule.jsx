...
export default function CostTemplatesModule({ onNavigate }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (slideOver === 'create') {
        const { error } = await supabase.from('cost_templates').insert(payload);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('cost_templates').update(payload).eq('id', slideOver.id);
        if (error) throw error;
      }
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xs font-extrabold tracking-[0.3em] uppercase text-brand-muted">PLANTILLAS COSTOS</h2>
              <p className="text-[10px] text-brand-muted/40 tracking-wide mt-0.5">Gastos operativos recurrentes (auto-poblados)</p>
            </div>
          </div>
          <button
            onClick={openCreate}
